...
Функция ПолучитьПредставлениеПолейДокументаВЗапрос() Экспорт
	...
	|ИЗ
	|	Документ.РасходнаяНакладная.ШтрихкодыУпаковок КАК РеализацияТоваровУслугШтрихкодыУпаковок
	|ГДЕ
	...
	|			ТОГДА 1
	|		ИНАЧЕ РеализацияТоваровУслугТовары.Номенклатура.Вес
	|	КОНЕЦ";
	...
	Результат.Вставить("ТекстПоляТаблицаТовары", "РасходнаяНакладная.Запасы");
	Результат.Вставить("ТекстПоляСерия", "Партия");
	...
	Результат.Вставить("ТекстПоляИсточник", "Документ.РасходнаяНакладная");
...
Функция НайтиПодчиненныйСчетФактура(ДокОснование) Экспорт
	...
	Возврат СчетаФактурыУНФ.ПолучитьПодчиненныйСчетФактуру(ДокОснование, Ложь);
...
Функция ПолучитьСведенияОбОрганизации(Организация) Экспорт
	...
	Возврат ПечатьДокументовУНФ.СведенияОЮрФизЛице(Организация, ТекущаяДатаСеанса());
...
Функция ОписаниеОрганизации(СписокСведений, Список = "") Экспорт
	...
	Возврат ПечатьДокументовУНФ.ОписаниеОрганизации(СписокСведений, Список);
...
Функция НайтиДоговорКонтрагента(Контрагент, Номер) Экспорт
	...
	Запрос.Текст = 
		...
		|	И ДоговорыКонтрагентов.НомерДоговора ПОДОБНО &Номер";
